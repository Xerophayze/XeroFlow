...
class AssistantNode(BaseNode):
    ...
    def create_monitor_window(self):
        ...
        if not hasattr(self, 'monitor_window') or not self.monitor_window:
            ...
            submit_tab.grid_rowconfigure(0, weight=0)  # Instructions label
            submit_tab.grid_rowconfigure(1, weight=2)  # Instructions text
            submit_tab.grid_rowconfigure(2, weight=0)  # Files label
            submit_tab.grid_rowconfigure(3, weight=1)  # Files list
            submit_tab.grid_rowconfigure(4, weight=0)  # Buttons
            ...
            instructions_label.grid(row=0, column=0, sticky="w", padx=5, pady=(5, 0))
            
            self.instructions_text = scrolledtext.ScrolledText(submit_tab, height=10, width=80, wrap=tk.WORD)
            self.instructions_text.grid(row=1, column=0, sticky="nsew", padx=5, pady=5)
            
            # Files section
            files_frame = ttk.Frame(submit_tab)
            files_frame.grid(row=2, column=0, sticky="ew", padx=5, pady=(10, 0))
            ...
            files_frame = ttk.Frame(submit_tab)
            files_frame.grid(row=3, column=0, sticky="nsew", padx=5, pady=5)
            ...
            buttons_frame.grid(row=4, column=0, sticky="ew", padx=5, pady=10)
    ...
    def submit_project(self):
        ...
        try:
            ...
            # Create a project folder in the inbox with timestamp and unique ID
    ...
    def process_audio_file(self, file_path):
        ...
        try:
            ...
            # Create transcription file in the project folder
            transcription_file = project_folder / f"{file_path.stem}_transcription.txt"
            ...
            print(f"[DEBUG] Processing complete transcription: {transcription_file}")
            self.process_text_file(str(transcription_file))
            
            # Move files to output folder
            if self.current_job_folder:
                # Move transcription file only, skip copying the original media file
                new_trans_location = Path(self.current_job_folder) / transcription_file.name
                shutil.move(str(transcription_file), str(new_trans_location))
    ...
    def scan_inbox_folders(self):
        ...
        try:
            ...
            if instructions_file.exists() and instructions_file not in self.processed_files:
                ...
                # Parse the instructions file to find referenced files before moving anything
                referenced_files = self.parse_instructions_file(instructions_file)
                
                # Only check if referenced files are ready if there are any references
                if referenced_files:
                    # Check if all referenced files are ready (not being modified)
                    ...
                    # If there are no referenced files, we can proceed immediately
                ...
                # Create a project folder for the instructions file and all referenced files
            ...
            # Handle other files directly in inbox
            ...
            for project_folder in inbox_folder.glob('**/'):
                if project_folder != inbox_folder:  # Skip inbox root
                    if not project_folder.exists():
                        print(f"[DEBUG] Skipping deleted folder during scan: {project_folder}")
                        continue
    ...
    def process_project_folder(self, project_folder):
        ...
        try:
            project_folder = Path(project_folder)  # Ensure it's a Path object
            
            # First check if folder still exists
            if not project_folder.exists():
                self.update_log(f"Project folder no longer exists: {project_folder}")
                print(f"[DEBUG] Project folder no longer exists: {project_folder}")
                return
            
            # Check for Assistant_instructions.txt first
            instructions_file = project_folder / "Assistant_instructions.txt"
            if instructions_file.exists():
                self.update_log(f"Found instructions file: {instructions_file.name}")
                print(f"[DEBUG] Found instructions file: {instructions_file}")
            ...
            print(f"[DEBUG] Found {total_files} files to process in folder: {project_folder}")
            
            for file_path in files_to_process:
                if file_path not in self.processed_files:
                    self.update_log(f"Processing file {processed_count + 1} of {total_files}: {file_path.name}")
                    print(f"[DEBUG] Processing file {processed_count + 1} of {total_files}: {file_path}")
                    
                    # Check if file is ready for processing
                    if not self.is_file_ready_for_processing(file_path):
                        self.update_log(f"File {file_path.name} is still being modified, waiting for sync to complete")
                        print(f"[DEBUG] File is still being modified, waiting for sync to complete")
                        continue
                    ...
                    try:
                        ...
                        # Store the output folder for this file
                        ...
                        if self.is_audio_file(file_path) or self.is_video_file(file_path):
                            ...
                            self.process_audio_file(str(file_path))
            ...
            # After all files are processed, delete the project folder
            if processed_count == total_files:
                print(f"[DEBUG] All {total_files} files processed, attempting to delete project folder")
                    ...
                    if self.is_audio_file(file_path) or self.is_video_file(file_path):
                        ...
                        if self.is_video_file(file_path):
                            ...
                            if self.extract_audio_from_video(file_path, audio_file):
                                ...
                                # Use process_audio_file to handle chunking
                                self.process_audio_file(str(audio_file))
                                
                                # Read the generated transcription file
                                transcription_file = temp_dir / f"{audio_file.stem}_transcription.txt"
                                if transcription_file.exists():
                                    with open(transcription_file, 'r', encoding='utf-8') as tf:
                                        transcript = tf.read()
                                else:
                                    transcript = "[ERROR] Transcription file not found."
                            ...
                            self.process_audio_file(str(file_path))
                            # Read the generated transcription file
                            transcription_file = temp_dir / f"{Path(file_path).stem}_transcription.txt"
                            if transcription_file.exists():
                                with open(transcription_file, 'r', encoding='utf-8') as tf:
                                    transcript = tf.read()
                            else:
    ...
    def transcribe_audio(self, file_path):
        ...
        # Look for OpenAI endpoint first
        interfaces = self.config.get('interfaces', {})
        whisper_endpoint = None
        
        # First try to find a specific Whisper endpoint
        for endpoint, config in interfaces.items():
            if 'whisper' in endpoint.lower():
                whisper_endpoint = endpoint
                break
        
        # If no specific Whisper endpoint, try to use any OpenAI endpoint
        ...
        if not whisper_endpoint:
            error_msg = "No OpenAI endpoint found for Whisper API. Available endpoints: " + ", ".join(interfaces.keys())
            print(f"[DEBUG] {error_msg}")
            raise Exception(error_msg)
            
        print(f"[DEBUG] Using whisper endpoint: {whisper_endpoint}")
        ...
        # Check for token usage in response and log it
        if isinstance(response, dict):
            if 'token_usage' in response:
                # Create a copy of token_usage to avoid modifying the original
                token_usage_data = response['token_usage'].copy() if isinstance(response['token_usage'], dict) else {}
                
                # Add audio duration to token usage data if available
                if 'audio_duration' in response:
                    token_usage_data['audio_duration'] = response['audio_duration']
                
                # Log the token usage with audio duration
                self.log_token_usage(whisper_endpoint, 'whisper-1', token_usage_data)
        ...
        if not response:
            raise Exception("No response from Whisper API")
        else:
                    ...
                    if file_path_obj.parent == inbox_folder:
                        ...
                        try:
                            if self.node.is_audio_file(file_path):
                                ...
                                self.node.process_audio_file(file_path)
    ...
    def send_to_api(self, prompt, api_endpoint):
        ...
        try:
            ...
            model = self.config.get('interfaces', {}).get(api_endpoint, {}).get('selected_model', '')
            request_data = {
                'content': prompt,
                ...
                'model': model,
                'max_tokens': self.config.get('interfaces', {}).get(api_endpoint, {}).get('max_tokens', 2000),
                'temperature': self.config.get('interfaces', {}).get(api_endpoint, {}).get('temperature', 0.7)
            }

            # Call the new process_api_request function
            response = process_api_request(
                api_endpoint,
                self.config,
                request_data,
            ...
            if isinstance(response, dict):
                if 'token_usage' in response:
                    self.log_token_usage(api_endpoint, model, response['token_usage'])
