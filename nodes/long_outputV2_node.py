...
class LongOutputV2Node(BaseNode):
    ...
    def check_rate_limits(self, prompt_tokens=0):
        ...
        if len(self.hourly_requests) >= self.rph_limit * 0.95:  # 95% of limit
            ...
            if wait_time > 0:
                print(f"[LongOutputNodeV2] Close to hourly limit ({len(self.hourly_requests)}/{self.rph_limit}). Waiting {wait_time:.1f}s...")
                time.sleep(wait_time)
        
        if len(self.request_times) >= self.rpm_limit * 0.95:  # 95% of limit
            wait_time = self.request_times[0] - minute_ago
            if wait_time > 0:
                print(f"[LongOutputNodeV2] Close to minute limit ({len(self.request_times)}/{self.rpm_limit}). Waiting {wait_time:.1f}s...")
                time.sleep(wait_time)
        ...
        if current_tpm + prompt_tokens > self.tpm_limit * 0.95:  # 95% of limit
            ...
            print(f"[LongOutputNodeV2] Close to token limit ({current_tpm}/{self.tpm_limit}). Waiting {wait_time:.1f}s...")
            time.sleep(wait_time)
    ...
    def adjust_rate_limits_from_response(self, response):
        ...
        if 'rate_limits' in response:
            ...
            # Log current limits
            print("\n[LongOutputNodeV2] Current API Rate Limits:")
            for key, value in limits.items():
                if value is not None:
                    print(f"[LongOutputNodeV2] {key}: {value}")
            ...
            if remaining_requests is not None and remaining_requests.isdigit():
                ...
                if remaining < 10:  # If we're running low on requests
                    print(f"[LongOutputNodeV2] Warning: Only {remaining} requests remaining!")
                    if reset_requests and reset_requests.isdigit():
                        wait_time = int(reset_requests)
                        print(f"[LongOutputNodeV2] Rate limit will reset in {wait_time} seconds")
                        return wait_time
            ...
            if remaining_tokens is not None and remaining_tokens.isdigit():
                ...
                if remaining < 1000:  # If we're running low on tokens
                    print(f"[LongOutputNodeV2] Warning: Only {remaining} tokens remaining!")
                    if reset_tokens and reset_tokens.isdigit():
                        wait_time = int(reset_tokens)
                        print(f"[LongOutputNodeV2] Token limit will reset in {wait_time} seconds")
                        return wait_time
    ...
    def process_with_retry(self, api_details, prompt, max_retries=3, timeout=None, cooldown=None, request_delay=None):
        ...
        for attempt in range(max_retries):
            try:
                ...
                print(f"[LongOutputNodeV2] Attempt {attempt + 1} failed: {str(e)}")
                if attempt < max_retries - 1:
                    # Wait based on the attempt number
                    wait_time = [30, 60, 120][attempt]
                    print(f"[LongOutputNodeV2] Waiting for {wait_time} seconds before retrying...")
                    time.sleep(wait_time)
                else:
                    raise e
    ...
    def _make_api_call(self, api_details, prompt):
        ...
        try:
            ...
            print("[LongOutputNodeV2] Making API call...")
            response = process_api_request(api_details, prompt)
            print("[LongOutputNodeV2] API call completed")
            
            return response
            
        except Exception as e:
            print(f"[LongOutputNodeV2] Exception in api_call: {str(e)}")
            return {'error': str(e)}
    ...
    def process(self, inputs):
        ...
        try:
            ...
            if use_array:
                ...
                print(f"[LongOutputNodeV2] Using temporary storage: {temp_file.name}")
            ...
            if not api_details:
                ...
                print(f"[LongOutputNodeV2] Error: {error_msg}")
...
if the section above starts with "(Continued)" then only include "(Continued) - " at the beginning of your output like this:  (Continued) - content......"""
                    ...
                    if 'error' in api_response:
                        ...
                        print(f"[LongOutputNodeV2] API Error: {error_msg}")
                        if use_array:
                    ...
                    print(f"[LongOutputNodeV2] API Response for item {i+1}: {response_text[:100]}...")
                ...
                try:
                    ...
                    print(f"[LongOutputNodeV2] Warning: Could not yield control: {str(e)}")
            ...
            if use_array and review_array:
                ...
                if reviewed_responses is None:
                    print("[LongOutputNodeV2] User cancelled during review")
            ...
            if temp_file:
                try:
                    ...
                    print("[LongOutputNodeV2] Cleaned up temporary storage")
                except Exception as e:
                    print(f"[LongOutputNodeV2] Warning: Could not clean up temp file: {str(e)}")
