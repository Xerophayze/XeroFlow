...
class NodeEditor:
    ...
    def check_connection_end(self, event):
        if self.connection_motion_bind:
            self.canvas.unbind("<B1-Motion>", self.connection_motion_bind)
            self.connection_motion_bind = None
        if self.connection_release_bind:
            self.canvas.unbind("<ButtonRelease-1>", self.connection_release_bind)
            self.connection_release_bind = None
        if self.temp_line:
    ...
    def clear_all_highlights(self):
        ...
        for node_id, node_data in self.nodes.items():
            ...
            if not node_data.get('selection_state', False):
    ...
    def remove_highlight(self, node_id):
        ...
        rect = self.nodes[node_id]['canvas_items']['rect']
        if not self.nodes[node_id].get('selection_state', False):
            self.canvas.itemconfig(rect, outline='black', width=2)
    ...
    def on_canvas_left_press(self, event):
        ...
        for item in items:
            tags = self.canvas.gettags(item)
            # Check for output connectors (they start connections)
            for tag in tags:
                if tag.startswith('output_'):
                    return
            if 'node' in tags or 'resize_handle' in tags or 'connector' in tags or 'drag_bar' in tags:
                return
    ...
    def on_canvas_left_drag(self, event):
        if not self.is_selecting or not self.selection_rect or not self.selection_start:
            return
    ...
    def on_canvas_left_release(self, event):
        ...
        if abs(max_x - min_x) < 4 and abs(max_y - min_y) < 4:
            return
        ...
        for node_id, node in self.nodes.items():
            ...
            if x + width < min_x or x > max_x or y + height < min_y or y > max_y:
                continue
            self.set_node_selected(node_id, True)

    def clear_selection(self):
        for node_id in list(self.selected_nodes):
            self.set_node_selected(node_id, False)
    ...
    def set_node_selected(self, node_id, selected):
        ...
        rect = node['canvas_items'].get('rect')
        if selected:
    ...
    def start_connection(self, event):
        ...
        if self.connection_release_bind:
            self.canvas.unbind("<ButtonRelease-1>", self.connection_release_bind)
        self.connection_motion_bind = self.canvas.bind("<B1-Motion>", self.update_temp_line, add="+")
        self.connection_release_bind = self.canvas.bind("<ButtonRelease-1>", self.check_connection_end, add="+")
        return "break"
